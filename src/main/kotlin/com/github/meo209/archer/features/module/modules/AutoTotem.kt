...
class AutoTotem : Module("Auto Totem", Category.PLAYER) {
    ...
    override fun register() {
        EventBus.global().handler(KeyPressEvent::class, { toggle() }, { enabled && it.key == key })

        EventBus.global().handler(PlayerInventorySlotChangeEvent::class, { event ->
            if (event.slot == 40 && event.old.item == Items.TOTEM_OF_UNDYING && MinecraftClient.getInstance().currentScreen == null) {
                InventoryUtils.moveFirstMatchingItem({ it.item == Items.TOTEM_OF_UNDYING }, 40)
            }
        }, { enabled })
    }
}